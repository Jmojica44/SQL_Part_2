...
SELECT lead_studio, count(favorite_movie_id) as count_of_fav_mov_id FROM movies 
LEFT JOIN users
ON movie_id = favorite_movie_id
GROUP BY lead_studio
ORDER BY count_of_fav_mov_id DESC LIMIT 1;

-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 5 >>>>>>>>>>>>>>>>>>>>>>>
-- Get the average Rotten Tomatoes score of all movies that are favorited by a user.
-- EXPECTED RESULT: 52.21

SELECT round(avg(rotten_tomatoes),2) as avg_rot_score FROM movies 
RIGHT JOIN users
ON favorite_movie_id = movie_id;
...
SELECT film, count(film) as count_of_film FROM movies
GROUP BY film
HAVING count_of_film > 1
